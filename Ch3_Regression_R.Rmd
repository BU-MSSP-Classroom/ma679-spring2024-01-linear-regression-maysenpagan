...
lmfit_2008<- lm(log(SalePrice) ~ log(`Lot Area`) + `Lot Config` + `Bldg Type` + `Overall Qual` + `TotRms AbvGrd` + `Garage Cars` + (`TotRms AbvGrd`*log(`Lot Area`)) + (`Garage Cars`*log(`Lot Area`)), data = ames_raw_2008)
...
~~~
There does appear to be a relationship between the predictor and the response. The p-value of the coefficient for mpg is very small suggesting that there is a significant association between mpg and horsepower. The negative sign of the coefficient means that the relationship is negative. As the horsepower increases, the mpg tends to decrease. The predicted mpg associated with a horsepower of 98 is about 24.47.
